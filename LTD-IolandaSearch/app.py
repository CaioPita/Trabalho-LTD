...
DATABASE = 'ltdAtualizado.db'             # Variavel com o nome do db
...
def pesquisar():
    if 'username' in session:  # Verifica se o usuário está logado
        termo_pesquisa = request.form.get('termo_pesquisa')  # Captura o termo de pesquisa enviado pelo formulário
        return redirect(url_for('pesquisa', termo_pesquisa=termo_pesquisa))  # Redireciona para a página de resultados
    return redirect(url_for('login'))  # Se não estiver logado, redireciona para login.html
...
@app.route('/pesquisa')
def pesquisa():
    termo_pesquisa = request.args.get('termo_pesquisa')  # Obtém o termo de pesquisa da URL
    conn = get_db_connection()  # Função de conexão com o banco de dados
    
    # Consulta para buscar produtos que contenham o termo pesquisado
    query = "SELECT * FROM tbl_produto WHERE titulo LIKE ?"
    produtos = conn.execute(query, ('%' + termo_pesquisa + '%',)).fetchall()
    conn.close()
    
    # Renderiza a página de pesquisa com os resultados
    return render_template('pesquisa.html', produtos=produtos, termo_pesquisa=termo_pesquisa)
